...
def render_model_eval_view(results_dir: Path, figures_dir: Path) -> None:
    ...
    if summary_csv.exists():
        ...
        st.dataframe(df_comp.style.highlight_min(axis=0, subset=['MAE', 'RMSE'], color='#c8e6c9').highlight_max(axis=0, subset=['R2'], color='#c8e6c9'), use_container_width=True)
    else:
    ...
    with col1:
        if fig_act_vs_pred.exists():
            st.image(str(fig_act_vs_pred), caption="Actual vs Predicted Growth Trajectory (LR vs LSTM)", use_column_width=True)
    with col2:
        if fig_residuals.exists():
            st.image(str(fig_residuals), caption="Residual Distributions & Error Scatter", use_column_width=True)

    if fig_loss.exists():
        st.image(str(fig_loss), caption="Stacked LSTM Training and Validation Loss Convergence", use_column_width=True)
